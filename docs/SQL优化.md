...
MVCC 是一种并发控制方法，它通过为数据行保存多个版本（快照），实现**读不加锁**且**读写不冲突**，从而提高数据库的并发性能。

* 读操作（SELECT）可以读取数据的历史快照，无需等待写操作释放锁；
* 写操作（UPDATE/DELETE）会创建数据的新版本，不影响其他事务的读操作。
...
1.  **隐藏列**：每行数据包含隐藏列`DB_TRX_ID`（最后修改该数据的事务 ID）和`DB_ROLL_PTR`（指向 undo log 中的历史版本）。
2.  **undo log**：保存数据的历史版本，事务读取时根据可见性规则选择合适的版本。
3.  **Read View（读视图）**：事务启动时生成的快照，用于判断数据版本对当前事务是否可见。
