...
5. 对索引字段进行计算操作、字段上使用函数。（索引为 emp(ename,empno,sal)）
...
一般来说，锁可以防止脏读、不可重复读和幻读。
 ...
 事务隔离五种级别：
        ...
        提交读(Read Committed)  防止脏读，最常用的隔离级别,并且是大多数数据库的默认隔离级别
        可重复读(Repeated Read)  可以防止脏读和不可重复读
        串行读(Serializable)  可以防止脏读，不可重复读取和幻读，(事务串行化)会降低数据库的效率
...
选用什么隔离级别，主要看应用场景，隔离级别越低，事务并发能力越高。MySQL默认的隔离级别是可重复读(Repeated Read)
